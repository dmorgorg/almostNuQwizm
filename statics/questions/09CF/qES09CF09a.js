...
QWIZM.question.qES09CF09a = function (qNumber) {
  var qId = 1000231; // question ID number, unique to this question    
  ...
  var ABy = lcrng.getNext(500, 1000, 10),
      ...
      theta = lcrng.getNext(25, 31, 0.1); //calcs

  var RE = sd(P * CDx / (sin(theta) * (CDy + DFy) - 2 * cos(theta) * ABx)),
      Fx = sd(RE * sin(theta)),
  ...
  RC = stringify(RC);
  RB = stringify(RB);
  var statement = "The frame illustrated is made up of three rigid structural members !$ABC, CDE!$ and !$BFD!$. All connections are pinned except for the frictionless roller at !$E!$. Determine the magnitude of the reaction at each connection.",
