...
QWIZM.question.qES09CF04a = function (qNumber) {
  ...
  var AD = stringify(lcrng.getNext(1.5, 3, 0.05)),
      ...
      DL = stringify(lcrng.getNext(1, 3, 0.05)); // convert to number equivalents of string inputs to avoid string concatenation
  // instead of addition!!
  // RC = sd(RC);
  ...
  var lenBD = sd(Math.sqrt(Math.pow(AB, 2) + Math.pow(AD, 2))),
      ...
      RD = sd(Math.sqrt(Math.pow(RDy, 2) + Math.pow(RDx, 2))),
      RDtheta = sd(atan(RDy / RDx)); //stringify - defaults to sigDigs
  ...
  RC = stringify(RC);
  RCtheta = stringify(RCtheta); // BF = stringify(BF);
  // CE = stringify(CE);
