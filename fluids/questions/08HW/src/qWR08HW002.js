...
QWIZM.question.qWR08HW002 = (qNumber) => {
    ...
    let len3 = Math.round(len2 * mult / 25) * 25,
        ...
        v1temp = Q1temp / 1000 / Math.PI / (d1 / 1000) ** 2 * 4,
        v2temp = Q2temp / 1000 / Math.PI / (d2 / 1000) ** 2 * 4,
        v3temp = Q3temp / 1000 / Math.PI / (d3 / 1000) ** 2 * 4,
        ...
        Q2 = Q2temp * scale2,
        Q3 = Q3temp * scale2,
        vel1 = Q1 / 1000 / Math.PI / (d1 / 1000) ** 2 * 4,
        vel2 = Q2 / 1000 / Math.PI / (d2 / 1000) ** 2 * 4,
        vel3 = Q3 / 1000 / Math.PI / (d3 / 1000) ** 2 * 4,

        hLBC = utils.fluids.getHeadLoss(len1, Q1, c1, d1);

    // round for correct solution
    Q1 = stringify(Q1);
    Q2 = stringify(Q2);
    Q3 = stringify(Q3);
    vel1 = stringify(vel1);
    ...
    let statement = `Water flows from !$A!$ to !$D!$, the flow splitting into three parallel pipes !$1, 2!$ and !$3!$ between !$B!$ and !$C!$. Flow through the system is !$${Q}!$ L/s. <p>Determine the flow and the average velocity through each of the pipes !$1!$, !$2!$ and !$3!$. <p>Also, determine the head loss between !$B!$ and !$C!$.<p>Disregard all minor losses.`;

    let img = `../../images/08HW/qwizmHWQ02.png`;

    let inputs = QWIZM.getInputOverlays([{
            input: `!$               
                    L_1 = ${len1} \\textsf{ m, } D_1 = ${d1} \\textsf{ mm, }C_1 = ${c1}                    
                    !$`,
            left: 56,
            top: 13,
            // fs: 80  // percentage
            ...
            input: `!$               
                    L_2 = ${len2} \\textsf{ m, } D_2 = ${d2} \\textsf{ mm, }C_2 = ${c2}                    
                    !$`,
            left: 54,
            top: 30,
            // fs: 80  // percentage
            ...
            input: `!$               
                    L_3 = ${len3} \\textsf{ m, } D_3 = ${d3} \\textsf{ mm, }C_3 = ${c3}                    
                    !$`,
            left: 54,
            top: 90,
            // fs: 80  // percentage
